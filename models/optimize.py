...
class Optimize(SetPath):
    ...
    def validate_sheet(self):
        ...
        for sheet in sheet_list:
            try:
                ...
                self.status['val_sheet'][sheet] = 1 if set(list(df.columns)) == set(sheet_dict[sheet]) else 0
            except Exception:
    ...
    def validate_master(self):
        ...
        self.df_cust = pd.read_excel(self.file, sheet_name='cust', dtype=dtype)
        self.df_mat = pd.read_excel(self.file, sheet_name='mat', dtype=dtype)
        self.df_truck = pd.read_excel(self.file, sheet_name='truck', dtype=dtype)
        self.df_plant = pd.read_excel(self.file, sheet_name='plant', dtype=dtype)
        self.df_order = pd.read_excel(self.file, sheet_name='order', dtype=dtype)
        self.df_custmat = pd.read_excel(self.file, sheet_name='custmat', dtype=dtype)
        self.df_custtruck = pd.read_excel(self.file, sheet_name='custtruck', dtype=dtype)
        self.df_custplant = pd.read_excel(self.file, sheet_name='custplant', dtype=dtype)
        self.df_truckplant = pd.read_excel(self.file, sheet_name='truckplant', dtype=dtype)
        self.df_matplant = pd.read_excel(self.file, sheet_name='matplant', dtype=dtype)
        ...
        self.truck = pd.Series(self.df_truck['truckname'].values, index=self.df_truck['truck']).to_dict()
        self.plant = pd.Series(self.df_plant['plantname'].values, index=self.df_plant['plant']).to_dict()
        ...
        self.mat_weight = pd.Series(self.df_mat['weight'].values, index=self.df_mat['mat']).to_dict()
        ...
        self.order_lot = dict((x[0], int(x[1] / self.mat_lot[x[0][1]]) * self.mat_lot[x[0][1]]) for x in self.order.items())  # check for full lot
        self.order_qty = dict((x[0], self.order[x[0]] - x[1]) for x in self.order_lot.items())  # check remainder
        self.order_priority = self.makedict(self.df_order, 'id', 'order_priority', self.custmat, 99999, 99999)
        ...
        self.sales_qty = self.makedict(self.df_custmat, 'id', 'sales_qty', self.custmat, 0.0001, 0.0001)
        ...
        self.max_weight = self.makedict(self.df_custtruck, 'id', 'max_weight', self.custtruck, 9999999, 0)
        self.trans_cost = self.makedict(self.df_custtruck, 'id', 'trans_cost', self.custtruck, 0, 0)
        self.extraplant_cost = self.makedict(self.df_custtruck, 'id', 'extraplant_cost', self.custtruck, 0, 0)
        self.cost_ratio = self.makedict(self.df_custtruck, 'id', 'cost_ratio', self.custtruck, 1, 1)
        self.plant_limit = self.makedict(self.df_custtruck, 'id', 'plant_limit', self.custtruck, 1, 1)

        # sheet custplant
        self.df_custplant['id'] = self.df_custplant.apply(lambda x: (x['cust'], x['plant']), axis=1)
        self.custplant_avl = self.makedict(self.df_custplant, 'id', 'custplant_avl', self.custplant, 0, 0)

        # sheet truckplant
        self.df_truckplant['id'] = self.df_truckplant.apply(lambda x: (x['truck'], x['plant']), axis=1)
        self.truckplant_avl = self.makedict(self.df_truckplant, 'id', 'truckplant_avl', self.truckplant, 0, 0)
        ...
        self.status['val_master']['order'] = self.checkmaster('order', self.df_order, ['cust', 'mat'], [self.cust, self.mat])
        self.status['val_master']['custmat'] = self.checkmaster('custmat', self.df_custmat, ['cust', 'mat'], [self.cust, self.mat])
        self.status['val_master']['custtruck'] = self.checkmaster('custtruck', self.df_custtruck, ['cust', 'truck'], [self.cust, self.truck])
        self.status['val_master']['custplant'] = self.checkmaster('custplant', self.df_custplant, ['cust', 'plant'], [self.cust, self.plant])
        self.status['val_master']['truckplant'] = self.checkmaster('truckplant', self.df_truckplant, ['truck', 'plant'], [self.mat, self.truck])
        self.status['val_master']['matplant'] = self.checkmaster('matplant', self.df_matplant, ['mat', 'plant'], [self.mat, self.plant])
    ...
    def validate_feas(self):
        ...
        df_feas_cust = pd.DataFrame([{'cust': i, 'truck': k} for i in self.cust for k in self.truck])
        ...
        df_feas_cust = pd.merge(df_feas_cust, self.df_truckplant.groupby(['truck'], as_index=False).agg({"truckplant_avl": "sum"}), on='truck', how='left')
        ...
        df_feas_custmat = pd.DataFrame([{'mat': j, 'plant': l} for j in self.mat for l in self.plant])
        ...
        df_feas_custmat = pd.merge(df_feas_custmat, self.df_custplant[self.df_custplant['custplant_avl'] == 1], on='plant', how='inner')
        ...
        df_feas_custtruck = pd.DataFrame([{'cust': i, 'truck': k, 'plant': l} for i in self.cust for k in self.truck for l in self.plant])
        ...
        df_feas_custtruck = pd.merge(df_feas_custtruck, self.df_custplant[self.df_custplant['custplant_avl'] == 1], on=['cust', 'plant'], how='left')
        df_feas_custtruck = pd.merge(df_feas_custtruck, self.df_truckplant[self.df_truckplant['truckplant_avl'] == 1], on=['truck', 'plant'], how='left')
    ...
    def optimize(self):
        ...
        solver.Minimize(solver.Sum([dos_below_min[x[0], x[1]] for x in dos_below_min]) * penalty_below_min # dos after below minimum dos
                        ...
                        + solver.Sum([order_lot_left[x[0], x[1]] / self.order_priority[x[0], x[1]] for x in order_lot_left]) * penalty_order_left  # Minimize order lot left
                        + solver.Sum([order_qty_left[x[0], x[1]] / self.order_priority[x[0], x[1]] for x in order_qty_left]) * penalty_order_left # Minimize order qty left
                        )
        ...
        for i in self.cust:
            for j in self.mat:
                ...
                solver.Add(self.order_lot[(i, j)]
                           - solver.Sum([trans_order_lot[x[0], x[1], x[2], x[3]] for x in trans_order_lot if (x[0] == i and x[1] == j)]) * self.mat_lot[j]
                           <= order_lot_left[(i, j)])
                solver.Add(self.order_lot[(i, j)]
                           - solver.Sum([trans_order_lot[x[0], x[1], x[2], x[3]] for x in trans_order_lot if (x[0] == i and x[1] == j)]) * self.mat_lot[j]
                           >= order_lot_left[(i, j)])
                ...
                solver.Add(self.order_qty[(i, j)]
                           - solver.Sum([trans_order_qty[x[0], x[1], x[2], x[3]] for x in trans_order_qty if (x[0] == i and x[1] == j)]) * self.order_qty[i, j]
                           <= order_qty_left[(i, j)])
                solver.Add(self.order_qty[(i, j)]
                           - solver.Sum([trans_order_qty[x[0], x[1], x[2], x[3]] for x in trans_order_qty if (x[0] == i and x[1] == j)]) * self.order_qty[i, j]
                           >= order_qty_left[(i, j)])
                ...
                solver.Add((self.inv_qty[(i, j)]
                            + solver.Sum([trans_stk_lot[x[0], x[1], x[2], x[3]] for x in trans_stk_lot if x[0] == i and x[1] == j]) * self.mat_lot[j]
                            ) / self.sales_qty[(i, j)]
                           <= dos_after[(i, j)])
                solver.Add((self.inv_qty[(i, j)]
                            + solver.Sum([trans_stk_lot[x[0], x[1], x[2], x[3]] for x in trans_stk_lot if x[0] == i and x[1] == j]) * self.mat_lot[j]
        ...
        for i in self.cust:
            for k in self.truck:
                ...
                # plant limit
                solver.Add(solver.Sum([plant_visit[x[0], x[1], x[2]] for x in plant_visit if (x[0] == i and x[1] == k)]) <= self.plant_limit[(i, k)])
        ...
        df_custtruck_output = pd.merge(self.df_custtruck.drop('id', axis=1), df_custtruck_output, on=['cust', 'truck'], how='left')
        if len(df_trans) > 0:
            df_custtruck_plant = df_trans[['cust', 'truck', 'plant']].drop_duplicates().groupby(['cust', 'truck'])['plant'].apply(list).reset_index()
            df_custtruck_output = pd.merge(df_custtruck_output, df_custtruck_plant, on=['cust', 'truck'], how='left')
        else:
        ...
        if len(df_trans) > 0:
            ...
            df_custmat_output = pd.merge(df_custmat_output, df_custmat_plant, on=['cust', 'mat'], how='left')
        else:
            df_custmat_output['plant'] = None
        df_custmat_plantavl = pd.merge(self.df_custplant[self.df_custplant['custplant_avl'] == 1].drop('id', axis=1), self.df_matplant.drop('id', axis=1), on=['plant'], how='left')
        df_custmat_plantavl = df_custmat_plantavl.groupby(['cust', 'mat'])['plant'].apply(list).reset_index().rename(columns={'plant': 'plant_avl'})
        df_custmat_output = pd.merge(df_custmat_output, df_custmat_plantavl, on=['cust', 'mat'], how='left')
