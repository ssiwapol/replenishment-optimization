...
TMP_DIR = './tmp'
path = optimize.SetPath(TMP_DIR)

@app.route('/', methods=("POST", "GET"))
@app.route('/index', methods=("POST", "GET"))
def home():
    opt = optimize.Optimize(TMP_DIR)
...
def download(filename):
    return send_file(os.path.join(path.path(), filename), attachment_filename=filename, as_attachment=True, cache_timeout=0)

@app.route("/output/<tablename>", methods=['GET', 'POST'])
def output(tablename):
    dtype = {"plant": str, "truck": str, "cust": str, "mat": str}
    df = pd.read_excel(os.path.join(path.path(), 'output.xlsx'), sheet_name=tablename, dtype=dtype)
